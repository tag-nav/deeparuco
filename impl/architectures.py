...
def unet_upsample(x, x_down, num_filters, batch_norm=False):
    x = Conv2DTranspose(
        num_filters, (3, 3), strides=(2, 2), padding="same", activation="relu"
    )(x)
...
def regressor_hmaps_unet(s_filters=8, levels=4, batch_norm=False):
    ...
    x_l = Conv2D(s_filters * 2**levels, (3, 3), padding="same", activation="relu")(
        down
    )
    ...
    output_layer = Conv2D(1, (3, 3), padding="same", activation="sigmoid")(up)
